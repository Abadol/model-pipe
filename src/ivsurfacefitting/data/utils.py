...
def df_to_tensor(
    indexcol: str, indeces: pd.Series, valuescols: list[str], df: pd.DataFrame
) -> torch.Tensor:
    """
    ...
    tensor = torch.tensor(
        filtered_df.loc[:, valuescols].to_numpy(), dtype=torch.float32
    ).reshape(n_indices, -1, len(valuescols))

    return tensor


def tensor_to_df(
    indexcol: str, indeces: pd.Series, valuescols: list[str], tensor: torch.Tensor
) -> pd.DataFrame:
    """
